...
## 简介
- [📷 一、天地一体化多模态协同感知与时空数据融合方向]
  - [多模态图像融合](#Category1-1)：
    整合光学、红外、雷达等不同类型图像，应用于卫星遥感监测、无人机多源情报收集，提升复杂环境下的图像信息完整性与准确性
  - [多曝光图像融合](#Category5)：
    解决卫星或无人机在高动态光照场景下的成像问题，如极地地区昼夜交替、城市高反光区域的清晰成像。
- [🤖 二、 空天智能信息处理与自主决策方向]
  - [LLM](#Category2-1)：用于处理航空航天领域的文本数据，如任务指令解析、卫星遥测数据报告生成。
  - [VLM](#Category2-2)：实现航空航天图像的语义理解，例如对卫星拍摄的地貌图像进行自动分类和特征提取。
  - [Video LM](#Category2-3)：对无人机拍摄的视频流进行实时分析，识别动态目标和异常事件。
  - [Agent](#Category2-4)：构建自主决策智能体，应用于无人机编队协同作业、卫星自主任务规划。
- [🚀 三、空间建模与定位方向]
  - [图像配准技术](#Category3)：
    整合光学、红外、雷达等不同类型图像，应用于卫星遥感监测、无人机多源情报收集，提升复杂环境下的图像信息完整性与准确性
  - [目标检测技术](#Category6)：
